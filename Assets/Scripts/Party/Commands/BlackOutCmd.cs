			...
			if (intoxication >= party.MaxIntoxication)
			{
                ...
                switch (Util.RNG.Generate(0, 10))
                {
		            ...
		            case 2:
		                ...
		                party.blackOutEffectAmount = -Util.RNG.Generate(20, 51);
                        if (inventory.Equipped.TryGetValue(ItemConsts.OUTFIT, out item))
                        {
                            ((OutfitVO)item).Novelty -= party.blackOutEffectAmount;
                        }
		                break;
		            case 3:
		                party.blackOutEffect = "Outfit Ruined";
                        AmbitionApp.SendMessage(InventoryMessages.DELETE_ITEM, inventory.Equipped[ItemConsts.OUTFIT]);
		                break;
		            case 4:
                        if (inventory.Equipped.TryGetValue(ItemConsts.ACCESSORY, out item) && item != null)
		                {
                            ...
                            AmbitionApp.SendMessage(InventoryMessages.DELETE_ITEM, item);
                        }
                        else
		                {
